...
fun_validate_act = function(B_ACTUAL, read_dir){
  ...
  if(length(which(is.na(data2$NOMBRE) == TRUE)) > 0)
    {stop(paste("Error en casilla NOMBRE, lineas: ", toString(which(is.na(data2$NOMBRE) == TRUE)+1)))}
  
  if(length(which(is.null(data2$CC) == TRUE)) > 0)
  ...
  if(length(which(is.na(data2$F_NAC) == TRUE)) > 0)
    {stop(paste("Error en F_NAC, lineas: ", toString(which(is.na(data2$F_NAC) == TRUE)+1)))}
  
  data2$F_ING = as.Date(data2$F_ING, format="%Y.%m.%d")
  data2$F_DES = as.Date(data2$F_DES, format="%Y.%m.%d")
  
  data2$N = as.numeric(data2$N)
  
  if(length(which(is.na(data2$N) == TRUE)) > 0)
    {stop(paste("No numerico en columna secuencial (N), lineas: ", toString(which(is.na(data2$N) == TRUE)+1)))}
  
  if(length(which(duplicated(data2[,c("N")]) == TRUE)) > 0)
    {stop(paste("Duplicado en columna secuencial (N), lineas: ", toString(which(duplicated(data2[,c("N")]) == TRUE)+1)))}
  
  #Transformamos a las cedulas en caracteres
 
  data2$CEDULA = as.character(data2$CEDULA)
  
  #Les quitamos los caracteres especiales
  
  data2$CEDULA = unlist(lapply(data2$CEDULA, extractNonNum))
  
  #Le pegamos un 0 a las que tienen 9 caracteres
  
  data2$CEDULA = ifelse(nchar(data2$CEDULA) == 9, 
                        paste("0", data2$CEDULA, sep=""), data2$CEDULA)
  
  nced = 9 
  data2$CEDULA2 = paste("0", substr(data2$CEDULA,(nchar(data2$CEDULA)+1)-nced,
                                    nchar(data2$CEDULA)), sep = "")
  
  Validacion21 = unlist(lapply(data2$CEDULA, funval))
  Validacion22 = unlist(lapply(data2$CEDULA2, funval))
  data2$CEDULA.C = ifelse(Validacion21 == "correcta", data2$CEDULA,
